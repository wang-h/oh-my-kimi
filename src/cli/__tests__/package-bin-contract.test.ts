...
describe('package bin contract', () => {
  it('declares omk with an explicit relative bin path and avoids packaging platform-specific native binaries', () => {
    ...
    assert.deepEqual(pkg.bin, { omk: 'dist/cli/omk.js' });
    assert.equal(pkg.scripts?.['build:explore'], 'cargo build -p omk-explore-harness');
    ...
    assert.equal(pkg.scripts?.['test:explore'], 'cargo test -p omk-explore-harness && node --test dist/cli/__tests__/explore.test.js dist/hooks/__tests__/explore-routing.test.js dist/hooks/__tests__/explore-sparkshell-guidance-contract.test.js');
    ...
    const binPath = join(process.cwd(), 'dist', 'cli', 'omk.js');
    ...
    const binEntry = results[0]?.files?.find((file) => file.path === 'dist/cli/omk.js');
    assert.ok(binEntry, 'expected npm pack output to include dist/cli/omk.js');

    const packagedHarnessPath = process.platform === 'win32' ? 'bin/omk-explore-harness.exe' : 'bin/omk-explore-harness';
    const packagedHarnessEntry = results[0]?.files?.find((file) => file.path === packagedHarnessPath);
    const packagedHarnessMetaEntry = results[0]?.files?.find((file) => file.path === 'bin/omk-explore-harness.meta.json');
    ...
    const crateManifestEntry = results[0]?.files?.find((file) => file.path === 'crates/omk-explore/Cargo.toml');
    const crateMainEntry = results[0]?.files?.find((file) => file.path === 'crates/omk-explore/src/main.rs');
    ...
    assert.ok(crateManifestEntry, 'expected npm pack output to include crates/omk-explore/Cargo.toml');
    assert.ok(crateMainEntry, 'expected npm pack output to include crates/omk-explore/src/main.rs');
  });
});
