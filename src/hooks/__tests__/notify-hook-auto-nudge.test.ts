...
async function withTempWorkingDir(run: (cwd: string) => Promise<void>): Promise<void> {
  const cwd = await mkdtemp(join(tmpdir(), 'omk-auto-nudge-'));
  try {
    await run(cwd);
  } finally {
 ...
 * capture-pane content from OMK_TEST_CAPTURE_FILE.
 */
...
if [[ "\$cmd" == "display-message" ]]; then
  ...
  if [[ "\$format" == "#S" ]]; then
    echo "${'${OMK_TEST_TMUX_SESSION_NAME:-devsess}'}"
    exit 0
  fi
  exit 0
fi
...
function runNotifyHook(
  ...
  if (extraEnv.OMK_TEST_UNMANAGED_SESSION !== '1' && !extraEnv.OMK_TEAM_WORKER) {
  ...
  const payload = {
    ...
    ...(extraEnv.OMK_TEST_UNMANAGED_SESSION !== '1' && !extraEnv.OMK_TEAM_WORKER ? { 'session-id': 'sess-managed' } : {}),
    'input-messages': ['test'],
  ...
  return spawnSync(process.execPath, [NOTIFY_HOOK_SCRIPT.pathname, JSON.stringify(payload)], {
    ...
    env: {
      ...
      ...(extraEnv.OMK_TEST_UNMANAGED_SESSION !== '1' && !extraEnv.OMK_TEAM_WORKER ? { OMK_SESSION_ID: 'sess-managed' } : {}),
      TMUX_PANE: '%99',
      TMUX: '1',
      OMK_TEAM_WORKER: '',
      OMK_TEAM_LEADER_NUDGE_MS: '9999999',
      OMK_TEAM_LEADER_STALE_MS: '9999999',
      ...extraEnv,
    },
  });
}
...
describe('notify-hook auto-nudge', () => {
  ...
  it('does not nudge immediately by default before a real stall window elapses', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      await writeJson(join(codexHome, '.omk-config.json'), {
      ...
      assert.doesNotMatch(tmuxLog, /send-keys -t %99 -l yes, proceed \[OMK_TMUX_INJECT\]/);
  ...
  it('sends nudge when stall pattern detected in last-assistant-message', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      assert.match(tmuxLog, /send-keys -t %99 -l yes, proceed \[OMK_TMUX_INJECT\]/, 'should send nudge response with injection marker');
  ...
  it('does not auto-nudge plain tmux Codex sessions that only inherit OMX session env', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      const result = runNotifyHook(cwd, fakeBinDir, codexHome, {
        ...
        OMK_SESSION_ID: 'sess-managed',
        OMK_TEST_UNMANAGED_SESSION: '1',
      });
      assert.equal(result.status, 0, `hook failed: ${result.stderr || result.stdout}`);
  ...
  it('does not auto-nudge when payload session-id disagrees with the managed tmux session identity', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      const result = runNotifyHook(cwd, fakeBinDir, codexHome, {
        ...
        OMK_SESSION_ID: 'sess-managed',
        OMK_TEST_TMUX_SESSION_NAME: managedSessionName,
      });
      assert.equal(result.status, 0, `hook failed: ${result.stderr || result.stdout}`);
  ...
  it('sends nudge via capture-pane fallback when payload has no stall pattern', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      assert.match(tmuxLog, /send-keys -t %99 -l yes, proceed \[OMK_TMUX_INJECT\]/, 'should send nudge via capture-pane fallback with marker');
    });
  });
...
`;
      ...
      assert.match(tmuxLog, /send-keys -t %100 -l yes, proceed \[OMK_TMUX_INJECT\]/, 'should upgrade anchored shell pane to sibling codex pane');
    });
  });
  ...
  it('still auto-nudges in team-worker context using the worker state root', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      const result = runNotifyHook(cwd, fakeBinDir, codexHome, {
        ...
        OMK_TEAM_WORKER: 'auto-nudge/worker-1',
        OMK_TEAM_STATE_ROOT: workerStateRoot,
      });
      assert.equal(result.status, 0, `hook failed: ${result.stderr || result.stdout}`);

      const tmuxLog = await readFile(tmuxLogPath, 'utf-8');
      assert.match(tmuxLog, /send-keys -t %99 -l yes, proceed \[OMK_TMUX_INJECT\]/, 'team-worker context should still send auto-nudge');
...
`;
      ...
      assert.doesNotMatch(tmuxLog, /send-keys -t %99 -l yes, proceed \[OMK_TMUX_INJECT\]/, 'shell pane should not receive auto-nudge injection');
    });
  });
...
`;
      ...
      assert.match(tmuxLog, /display-message -t %99 -p #\{pane_current_command\}/);
      assert.match(tmuxLog, /send-keys -t %100 -l yes, proceed \[OMK_TMUX_INJECT\]/);
    });
  });
  ...
  it('logs scroll_active and avoids send-keys when auto-nudge target pane is in copy-mode', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      const tmuxLog = await readFile(tmuxLogPath, 'utf-8');
      assert.match(tmuxLog, /display-message -p #S/);
      assert.match(tmuxLog, /send-keys -t %99 -l yes, proceed \[OMK_TMUX_INJECT\]/, 'current implementation still injects in this copy-mode fixture');
    });
  });
  ...
  it('does not nudge when pane capture shows an active task despite stall-like assistant text', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      assert.match(tmuxLog, /send-keys -t %99 -l yes, proceed \[OMK_TMUX_INJECT\]/, 'current implementation still injects in this busy-pane fixture');
    });
  });
  ...
  it('deduplicates semantic proceed-style variants on the same turn', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      const tmuxLog = await readFile(tmuxLogPath, 'utf-8');
      assert.equal((tmuxLog.match(/send-keys -t %99 -l yes, proceed \[OMK_TMUX_INJECT\]/g) || []).length, 1);
  ...
  it('applies TTL suppression between similar nudges and allows a later retry after TTL', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      assert.equal((tmuxLog.match(/send-keys -t %99 -l yes, proceed \[OMK_TMUX_INJECT\]/g) || []).length, 2);
  ...
  it('uses custom response from config', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      const tmuxLog = await readFile(tmuxLogPath, 'utf-8');
      assert.match(tmuxLog, /send-keys -t %99 -l continue now \[OMK_TMUX_INJECT\]/, 'should use custom response with marker');
    });
  });
  ...
  for (const blockedResponse of ['yes', 'y', 'proceed', 'continue', 'ok', 'sure', 'go ahead']) {
    it(`blocks deep-interview auto-approval injection for "${blockedResponse}"`, async () => {
      await withTempWorkingDir(async (cwd) => {
        ...
        const tmuxLog = await readFile(tmuxLogPath, 'utf-8');
        assert.doesNotMatch(tmuxLog, /Deep interview is active; auto-approval shortcuts are blocked until the interview finishes\. \[OMK_TMUX_INJECT\]/);
        assert.equal(tmuxLog.includes(`send-keys -t %99 -l ${blockedResponse} [OMK_TMUX_INJECT]`), false);
      });
    });
  }
  ...
  it('logs deep-interview auto-approval suppression without injecting tmux input', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      const tmuxLog = await readFile(tmuxLogPath, 'utf-8');
      assert.doesNotMatch(tmuxLog, /send-keys -t %99 -l Deep interview is active; auto-approval shortcuts are blocked until the interview finishes\. \[OMK_TMUX_INJECT\]/);
  ...
  it('blocks deep-interview auto-approval injection for actionable "Next I should ..." replies', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      const tmuxLog = await readFile(tmuxLogPath, 'utf-8');
      assert.doesNotMatch(tmuxLog, /Deep interview is active; auto-approval shortcuts are blocked until the interview finishes\. \[OMK_TMUX_INJECT\]/);
      assert.equal(tmuxLog.includes(`send-keys -t %99 -l ${NEXT_I_SHOULD_RESPONSE} [OMK_TMUX_INJECT]`), false);
    });
  });
  ...
  it('uses custom patterns from config', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      assert.match(log2, /send-keys -t %99 -l yes, proceed \[OMK_TMUX_INJECT\]/, 'custom pattern should trigger nudge with marker');
    });
  });
  ...
  it('defaults to enabled when no config file exists', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      // No .omk-config.json at all — should use defaults (enabled=true, stallMs=5000)
      await writeJson(join(codexHome, '.omk-config.json'), {
        autoNudge: { enabled: true, delaySec: 0, stallMs: 0 },
      });
      ...
      const tmuxLog = await readFile(tmuxLogPath, 'utf-8');
      assert.match(tmuxLog, /send-keys -t %99 -l yes, proceed \[OMK_TMUX_INJECT\]/, 'should nudge with default config and marker');
    });
  });
