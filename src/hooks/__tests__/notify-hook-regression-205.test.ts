...
async function withTempWorkingDir(run: (cwd: string) => Promise<void>): Promise<void> {
  const cwd = await mkdtemp(join(tmpdir(), 'omk-regression-205-'));
  try {
    await run(cwd);
  } finally {
...
function runNotifyHook(
  ...
  return spawnSync(process.execPath, [NOTIFY_HOOK_SCRIPT.pathname, JSON.stringify(payload)], {
    ...
    env: {
      ...
      OMK_SESSION_ID: 'sess-managed-regression',
      TMUX_PANE: '%99',
      TMUX: '1',
      OMK_TEAM_WORKER: '',
...
describe('regression-205: detectStallPattern matches "if you want"', () => {
  ...
  it('ignores OMX injection-marker lines when matching patterns', async () => {
    const { detectStallPattern, DEFAULT_STALL_PATTERNS } = await loadModule('notify-hook/auto-nudge.js');
    assert.equal(
      detectStallPattern('keep going [OMK_TMUX_INJECT]', DEFAULT_STALL_PATTERNS),
      false,
    );
  });
...
describe('regression-205: notify-hook records pending stall state on "if you want" by default', () => {
  ...
  before(() => {
    originalTeamWorker = process.env.OMK_TEAM_WORKER;
    originalTeamStateRoot = process.env.OMK_TEAM_STATE_ROOT;
    delete process.env.OMK_TEAM_WORKER;
    delete process.env.OMK_TEAM_STATE_ROOT;
  });

  after(() => {
    if (originalTeamWorker === undefined) delete process.env.OMK_TEAM_WORKER;
    else process.env.OMK_TEAM_WORKER = originalTeamWorker;
    if (originalTeamStateRoot === undefined) delete process.env.OMK_TEAM_STATE_ROOT;
    else process.env.OMK_TEAM_STATE_ROOT = originalTeamStateRoot;
  });
  ...
  it('records pending stall state instead of injecting immediately', async () => {
    await withTempWorkingDir(async (cwd) => {
      ...
      assert.doesNotMatch(
        tmuxLog,
        /send-keys -t %99 -l yes, proceed \[OMK_TMUX_INJECT\]/,
