...
const OMK_TEAM_WORKER_CLI_ENV = 'OMK_TEAM_WORKER_CLI';
const OMK_TEAM_WORKER_CLI_MAP_ENV = 'OMK_TEAM_WORKER_CLI_MAP';
const OMK_TEAM_WORKER_LAUNCH_MODE_ENV = 'OMK_TEAM_WORKER_LAUNCH_MODE';
const OMK_TEAM_AUTO_INTERRUPT_RETRY_ENV = 'OMK_TEAM_AUTO_INTERRUPT_RETRY';
...
export function resolveTeamWorkerLaunchMode(
  ...
  const raw = String(env[OMK_TEAM_WORKER_LAUNCH_MODE_ENV] ?? 'interactive').trim().toLowerCase();
  if (raw === '' || raw === 'interactive') return 'interactive';
  if (raw === 'prompt') return 'prompt';
  throw new Error(`Invalid ${OMK_TEAM_WORKER_LAUNCH_MODE_ENV} value "${env[OMK_TEAM_WORKER_LAUNCH_MODE_ENV]}". Expected: interactive, prompt`);
}
...
export function resolveTeamWorkerCli(launchArgs: string[] = [], env: NodeJS.ProcessEnv = process.env): TeamWorkerCli {
  const mode = normalizeTeamWorkerCliMode(env[OMK_TEAM_WORKER_CLI_ENV]);
...
export function resolveTeamWorkerCliPlan(
  ...
  const rawMap = String(env[OMK_TEAM_WORKER_CLI_MAP_ENV] ?? '').trim();
  ...
  if (entries.length !== 1 && entries.length !== workerCount) {
    throw new Error(
      `Invalid ${OMK_TEAM_WORKER_CLI_MAP_ENV} length ${entries.length}; `
  ...
  return expanded.map((entry) => {
    const mode = normalizeTeamWorkerCliMode(entry, OMK_TEAM_WORKER_CLI_MAP_ENV);
...
function buildModelInstructionsOverride(cwd: string, env: NodeJS.ProcessEnv): string {
  const filePath = translatePathForMsys(env[OMK_MODEL_INSTRUCTIONS_FILE_ENV] || join(cwd, 'AGENTS.md'));
...
export function buildWorkerProcessLaunchSpec(
  ...
  const workerEnv: Record<string, string> = {
    OMK_TEAM_WORKER: `${teamName}/worker-${workerIndex}`,
    [OMK_LEADER_NODE_PATH_ENV]: resolveLeaderNodePath(),
    [OMK_LEADER_CLI_PATH_ENV]: resolvedCliPath,
  };
...
export function createTeamSession(
  ...
  try {
    ...
    // Opt-out: set OMK_TEAM_MOUSE=0 in the environment.
    if (process.env.OMK_TEAM_MOUSE !== '0') {
...
function dismissClaudeBypassPermissionsPromptIfPresent(target: string, captured: string): boolean {
  if (process.env.OMK_TEAM_AUTO_ACCEPT_BYPASS === '0') return false;
...
function resolveSendStrategyFromEnv(): 'auto' | 'queue' | 'interrupt' {
  const raw = String(process.env.OMK_TEAM_SEND_STRATEGY || '')
    .trim()
    .toLowerCase();
...
function resolveWorkerCliFromMapForSend(
  ...
  try {
    const mode = normalizeTeamWorkerCliMode(selectedRaw, OMK_TEAM_WORKER_CLI_MAP_ENV);
 ...
 * 2) per-worker OMK_TEAM_WORKER_CLI_MAP entry (worker index aware)
 * 3) global/default OMK_TEAM_WORKER_CLI behavior
 */
...
export function waitForWorkerReady(
  ...
  const check = (): boolean => {
    ...
    if (paneHasTrustPrompt(result.stdout)) {
      ...
      // Opt-out by setting OMK_TEAM_AUTO_TRUST=0.
      if (process.env.OMK_TEAM_AUTO_TRUST !== '0') {
        sendRobustEnter();
        promptDismissed = true;
        return false;
      }
 ...
 * Opt-out: set OMK_TEAM_AUTO_TRUST=0 to disable auto-dismissal.
 */
export function dismissTrustPromptIfPresent(
  ...
  if (process.env.OMK_TEAM_AUTO_TRUST === '0') return false;
...
export async function sendToWorker(
  ...
  const allowAutoInterruptRetry = process.env[OMK_TEAM_AUTO_INTERRUPT_RETRY_ENV] !== '0';
  ...
  // If you need strictness for debugging, set OMK_TEAM_STRICT_SUBMIT=1.
  const strict = process.env.OMK_TEAM_STRICT_SUBMIT === '1';
  if (strict) {
...
// List all tmux sessions matching omk-team-* pattern
