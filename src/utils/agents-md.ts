export const OMK_GENERATED_AGENTS_MARKER = '<!-- omx:generated:agents-md -->'
const OMK_GENERATED_AGENTS_TITLE =
...
export function isOmxGeneratedAgentsMd(content: string): boolean {
  return (
    content.includes(OMK_GENERATED_AGENTS_MARKER) ||
    content.includes(OMK_GENERATED_AGENTS_TITLE)
  )
}

export function addGeneratedAgentsMarker(content: string): string {
  if (content.includes(OMK_GENERATED_AGENTS_MARKER)) return content
