...
const CODEX_BIN_ENV: &str = "OMK_EXPLORE_CODEX_BIN";
const HARNESS_ROOT_ENV: &str = "OMK_EXPLORE_ROOT";
...
fn usage() -> &'static str {
    "Usage: omk-explore --cwd <dir> --prompt <text> --prompt-file <explore-prompt.md> --model-spark <model> --model-fallback <model>"
}
...
fn temp_output_path() -> PathBuf {
    let nanos = SystemTime::now()
        ...
        .unwrap_or_default()
        .as_nanos();
    env::temp_dir().join(format!("omk-explore-{}-{}.md", std::process::id(), nanos))
}
...
mod tests {
    ...
    fn discover_codex_support_dirs_includes_home_omx_and_codex_when_present() {
        ...
        create_dir_all(home_dir.join(".omk")).expect("create .omk");
        ...
        assert_eq!(dirs, vec![home_dir.join(".omk"), home_dir.join(".codex")]);
    }
