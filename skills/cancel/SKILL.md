---
name: cancel
description: Cancel any active OMK mode (autopilot, ralph, ultrawork, ecomode, ultraqa, swarm, ultrapilot, pipeline, team)
---

# Cancel Skill

Intelligent cancellation that detects and cancels the active OMK mode.

**The cancel skill is the standard way to complete and exit any OMK mode.**
...
4. Team artifacts (`.omk/state/team/*/`, tmux sessions matching `omk-team-*`) are best-effort cleared as part of the legacy fallback.
...
After graceful pass:
  ...
  2. Destroy the tmux session: tmux kill-session -t omk-team-{name}
```
...
5. Destroy tmux session: `tmux kill-session -t omk-team-{name}`
...
```bash
echo "No active OMK modes detected."
echo ""
echo "Checked for:"
...
5. Team artifacts (`.omk/state/team/*/`, tmux sessions matching `omk-team-*`) remain best-effort cleanup items invoked during the legacy/global pass.
...
| Force | "All OMK modes cleared. You are free to start fresh." |
| None | "No active OMK modes detected." |
...
1. **Kill all team tmux sessions**: `tmux list-sessions -F '#{session_name}' 2>/dev/null | grep '^omk-team-'` and kill each
