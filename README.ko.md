...
> **[Website](https://github.com/wang-h/oh-my-kimi)** | **[Documentation](https://github.com/wang-h/oh-my-kimi#readme)** | **[CLI Reference](https://github.com/wang-h/oh-my-kimi#command-reference)** | **[Workflows](https://github.com/wang-h/oh-my-kimi#recommended-workflows)** | **[OpenClaw 통합 가이드](./docs/openclaw-integration.ko.md)** | **[GitHub](https://github.com/wang-h/oh-my-kimi)** | **[npm](https://www.npmjs.com/package/oh-my-kimi)**

[Kimi Code CLI](https://moonshotai.github.io/kimi-cli/en/)를 위한 멀티 에이전트 오케스트레이션 레이어.

## v0.9.0 새로운 기능 — Spark Initiative

Spark Initiative는 OMX의 네이티브 탐색/검사 경로를 강화한 릴리스입니다.

- **`omk explore` 네이티브 하네스** — 읽기 전용 저장소 탐색을 Rust 기반 하네스로 더 빠르고 엄격하게 실행합니다.
- **`omk sparkshell`** — 긴 출력을 요약하고 tmux pane 캡처를 지원하는 운영자용 네이티브 검사 표면입니다.
- **크로스 플랫폼 네이티브 릴리스 자산** — `omk-explore-harness`, `omk-sparkshell`, `native-release-manifest.json` 기반 hydration 경로가 릴리스 파이프라인에 포함됩니다.
...
User
  -> Kimi Code CLI
    ...
    -> .omk/ (런타임 상태, 메모리, 계획, 로그)
```

## 주요 명령어

```bash
omk                # Codex 실행 (tmux에서 HUD와 함께)
omk setup          # 범위별 프롬프트/스킬/설정 설치 + 프로젝트 .omk + 범위별 AGENTS.md
omk doctor         # 설치/런타임 진단
omk doctor --team  # Team/swarm 진단
omk team ...       # tmux 팀 워커 시작/상태/재개/종료
omk status         # 활성 모드 표시
omk cancel         # 활성 실행 모드 취소
omk reasoning <mode> # low|medium|high|xhigh
omk tmux-hook ...  # init|status|validate|test
omk hooks ...      # init|status|validate|test (플러그인 확장 워크플로우)
...
OMX는 이제 플러그인 스캐폴딩 및 검증을 위한 `omk hooks`를 포함합니다.

- `omk tmux-hook`은 계속 지원되며 변경되지 않았습니다.
- `omk hooks`는 추가적이며 tmux-hook 워크플로우를 대체하지 않습니다.
- 플러그인 파일은 `.omk/hooks/*.mjs`에 위치합니다.
- 플러그인은 기본적으로 비활성화되어 있으며; `OMK_HOOK_PLUGINS=1`로 활성화합니다.
...
```bash
OMK_BYPASS_DEFAULT_SYSTEM_PROMPT=0 omk     # AGENTS.md 주입 비활성화
...
```bash
OMK_TEAM_WORKER_CLI=auto    # 기본값; worker --model에 "claude"가 포함되면 claude 사용
OMK_TEAM_WORKER_CLI=codex   # Kimi Code CLI 워커 강제
OMK_TEAM_WORKER_CLI=claude  # Claude CLI 워커 강제
OMK_TEAM_WORKER_CLI_MAP=codex,codex,claude,claude  # 워커별 CLI 혼합 (길이=1 또는 워커 수)
OMK_TEAM_AUTO_INTERRUPT_RETRY=0  # 선택: 적응형 queue->resend 폴백 비활성화
```

참고:
- 워커 시작 인수는 여전히 `OMK_TEAM_WORKER_LAUNCH_ARGS`를 통해 공유됩니다.
- `OMK_TEAM_WORKER_CLI_MAP`은 워커별 선택을 위해 `OMK_TEAM_WORKER_CLI`를 재정의합니다.
...
- `.omk/setup-scope.json` (저장된 설정 범위)
- 범위에 따른 설치:
...
- 시작 동작: 저장된 범위가 `project`이면, `omk` 시작 시 자동으로 `CODEX_HOME=./.codex`를 사용합니다 (`CODEX_HOME`이 이미 설정되지 않은 경우).
...
- **[전체 문서](https://github.com/wang-h/oh-my-kimi#readme)** — 완전한 가이드
- **[CLI 레퍼런스](https://github.com/wang-h/oh-my-kimi#command-reference)** — 모든 `omk` 명령어, 플래그 및 도구
- **[알림 가이드](https://github.com/wang-h/oh-my-kimi#notifications)** — Discord, Telegram, Slack 및 webhook 설정
- **[권장 워크플로우](https://github.com/wang-h/oh-my-kimi#recommended-workflows)** — 일반적인 작업을 위한 실전 검증된 스킬 체인
- **[릴리스 노트](https://github.com/wang-h/oh-my-kimi#release-notes)** — 각 버전의 새로운 기능
...
[oh-my-claudecode](https://github.com/Yeachan-Heo/oh-my-claudecode)에서 영감을 받아 Kimi Code CLI용으로 적응하였습니다.
