...
Spark Initiative, OMK içindeki native keşif ve inceleme yolunu güçlendiren sürümdür.

- **`omk explore` için native harness** — salt okunur depo keşfini Rust tabanlı daha hızlı ve daha sıkı bir yol üzerinden çalıştırır.
- **`omk sparkshell`** — uzun çıktıları özetleyen ve açık tmux pane yakalama desteği veren operatör odaklı native inceleme yüzeyidir.
- **Çapraz platform native release varlıkları** — `omk-explore-harness`, `omk-sparkshell` ve `native-release-manifest.json` için hydration yolu artık release pipeline'ın parçasıdır.
...
omk setup          # Prompt/skill/config'i kapsama göre kur + proje .omk + kapsama özel AGENTS.md
...
OMK_TEAM_WORKER_CLI=codex   # Kimi Code CLI çalışanlarını zorla
...
- **[CLI Referansı](https://github.com/wang-h/oh-my-kimi#command-reference)** — Tüm `omk` komutları, bayraklar ve araçlar
