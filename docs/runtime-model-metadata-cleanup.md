...
Benchmark runs showed spawned child agents being labeled with legacy model aliases such as `sonnet`, even when OMK was launched under Codex with GPT-5.x reasoning settings.
...
After rebuilding and rerunning `omk setup --force`, spawned-agent metadata should stop surfacing `haiku` / `sonnet` / `opus` as if they were active runtime models for Codex-based OMK runs.

## Re-test steps

```bash
npm run build   # TypeScript build
node bin/omk.js setup --scope project --force
```
